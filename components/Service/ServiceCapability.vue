  ...
  <div class="service-capability">
    <div v-if=" type === 'devices.capabilities.color_setting'" class="service-capability__control --color">
      ...
      <color-input
        id="color"
        ref="color"
        v-model="capability.hsv"
        :style="`--bright:${capability.hsv.v>1.1?capability.hsv.v/100:capability.hsv.v}`"
        format="hsv object"
        position="bottom right"
        disable-alpha
        disable-text-inputs
        autofocus
        @input="updateDevice({type,value:capability.hsv})"
      />
    </div>
    ...
    <div
      v-if="type === 'devices.capabilities.range' &&
        (instance?.includes('bright')||instance?.includes('temp'))"
    ...
    <!--    <div-->
    <!--      v-if="type === 'devices.capabilities.range' &&-->
    <!--        instance?.includes('temp')"-->
    <!--      class="service-capability__control &#45;&#45;range &#45;&#45;temperature"-->
    <!--    >-->
    <!--      <slider-->
    <!--        v-model="capability.value" :min="range.min"-->
    <!--        :max="range.max"-->
    <!--        :step="range.precision"-->
...
const slider = useNuxtApp().$slider
